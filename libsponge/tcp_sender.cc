...
void TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
    uint64_t abs_ackno = unwrap(ackno, _isn, _previous_ackno);
    if (abs_ackno > _next_seqno || abs_ackno < _previous_ackno) {
        // cout << "invalid ackno!\n";
        return;
    }
    ...
    if (!_segments_outstanding.empty()) {
        ...
        _receiver_free_space = window_size + abs_ackno - _next_seqno;
    }
