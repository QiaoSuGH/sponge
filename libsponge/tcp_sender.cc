...
void TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
    ...
    uint16_t tmp_window;
    tmp_window = (window_size == 0 ? 1 : window_size);
    _receiver_window_size = window_size;
   if(static_cast<uint64_t>(tmp_window) + abs_ackno > _next_seqno)
        _sender_window_size = static_cast<uint64_t>(tmp_window) + abs_ackno - _next_seqno;
    else _sender_window_size = 0;
    fill_window();
    return;
}
...
void TCPSender::tick(const size_t ms_since_last_tick) {
    ...
    if(_ms_accumulated >= _current_retransmission_timeout){
